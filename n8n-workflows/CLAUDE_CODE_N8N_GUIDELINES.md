...
1. **Inline literal** when the value is stable and non-secret:
   ```js
      const lmsType = 'MOODLE';
         ...
         3. **n8n Credentials** for anything sensitive (API keys, tokens). Never hard-code secrets, never place them in query params, never read them from env.
         4. **n8n Variables** (`$vars.LMS_TYPE`) if the plan tier supports them — check before using; many Cloud tiers do not include Variables.

         **Forbidden patterns to grep for before commit:**
                                       ...
                                       if (!r.ok) { console.error(`FAIL ${r.status}  ${u}`); failed++; }
                                                 ...
                                                 console.error(`ERR          ${u}  ${e.message}`); failed++;
                                                   ...
                                                   **Rule:** generate a single shared `workflow-error-notifier.json` that:
                                                   ...
                                                   2. Formats the error, workflow name, node name, execution URL.
                                                   3. Sends it via the project Gmail credential (`id: 1lLnVsSKehWVnwnW`) to `richardknapp134@gmail.com`, subject prefix `[n8n ERROR]`.

                                                   Then in every other workflow's top-level `settings` block include:
                                                             ...
                                                             "connections": {...},
                                                               "settings": { "executionOrder": "v1" },
                                                                 ...
                                                                 **Rule:** every PR that touches `n8n-workflows/*.json` must pass:
                                                                 ...
                                                                 2. A schema sanity check: all Code nodes have `typeVersion: 1` (rule #2), no `process.env` anywhere (rule #1), all scheduleTriggers have `triggerAtDay` as array (rule #3).
                                                                 3. A smoke import: the JSON must parse and contain the top-level keys `name`, `nodes`, `connections` at minimum.

                                                                 Recommended single guard script: `scripts/lint-workflows.mjs` that enforces all three. Run it in CI.

                                                                 ---

                                                                 ## 11. Secrets policy

                                                                 **Why:** Obvious, but worth stating because n8n workflows sometimes tempt you to inline tokens.

                                                                 **Rule:** no secret value (API key, OAuth token, webhook verify token, database URL, bearer token) may appear in any committed workflow JSON. Secrets live in n8n Credentials only. If a workflow needs a verify token (e.g. WhatsApp webhook handshake), reference it via a Credential, not a hard-coded string.

                                                                 When Claude Code generates a workflow that requires a new secret, it must (a) leave a placeholder like `REPLACE_WITH_CREDENTIAL_ID` and (b) add a README note listing what the human operator must create in n8n before import.
                                                                 ...
                                                                 | Date | Rule | Origin |
                                                                 |---|---|---|
                                                                 | 2026-04-17 | Rules 1–12 established | Post-mortem of 6 failed production executions (DfE 404, EquiSmile typeVersion-2 crash, LMS Sync `process.env`) |
